...
func NewRouter(cfg config.Config, log *slog.Logger, deps Dependencies) *fiber.App {
	...
	v1.Head("/content/:contentID/download", rate("download_public", cfg.RateLimits.DownloadPublic), downloadHandler.Public)
	...
	authGroup.Post("/password/confirm", rate("auth_email_verify", cfg.RateLimits.AuthEmailVerify), authHandler.ConfirmPasswordChange)
	authGroup.Post("/email/change/confirm", rate("auth_email_verify", cfg.RateLimits.AuthEmailVerify), authHandler.ConfirmEmailChange)
